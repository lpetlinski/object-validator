    ...
    for(var field in validationRules) {
        if(field !== INDEX_BY_ARRAY && validationRules.hasOwnProperty(field)) {
            ...
            if(result !== false) {
                ...
                errors[removeSuffix(field)] = result;
            }
        }
    }
    return errors;
...
function processFieldValidation(obj, field, fieldValidationRules) {
    if(isArrayOfObjectsField(field)) {
        var fieldName = removeSuffix(field);
        return validateArrayOfObjects(obj, fieldName, fieldValidationRules);
    } else if(isSimpleArrayField(field)) {
        var fieldName = removeSuffix(field);
...
function isArrayOfObjectsField(fieldName) {
    return Utils.endsWith(fieldName, ARRAY_OBJECTS_SUFFIX);
}

function isSimpleArrayField(fieldName) {
    return Utils.endsWith(fieldName, SIMPLE_ARRAY_SUFFIX);
}

function isObjectField(fieldName) {
    return Utils.endsWith(fieldName, OBJECT_SUFFIX)
}

function removeSuffix(fieldName) {
    if(isArrayOfObjectsField(fieldName)) {
        return Utils.removeSuffix(fieldName, ARRAY_OBJECTS_SUFFIX);
    } else if(isSimpleArrayField(fieldName)) {
        return Utils.removeSuffix(fieldName, SIMPLE_ARRAY_SUFFIX);
    } else if(isObjectField(fieldName)) {
        return Utils.removeSuffix(field, OBJECT_SUFFIX);
    }
    return fieldName;
}
